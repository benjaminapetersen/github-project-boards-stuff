...
func estimateQueryCost(config Config) {
	...
	fmt.Println("  Read queries cost 1 point each. Mutations cost ~1-5 points.")
	fmt.Println()

	fmt.Println("    1 point  - List org projects")
	gqlTotal++

	fmt.Println("    N points - Fetch items per project (1 point per page of 100)")
	gqlTotal += 5 // rough estimate
